...
class atomic_forward_list {
  ...
  class my_iterator {
   public:
    ...
    reference operator*() { return *ref(m_data); }

    pointer operator->() { return &*ref(m_data); }
    ...
    bool operator!=(const my_iterator& it) const { return m_data != it.m_data; }
 ...
 public:
  ...
  auto front() { return ref(std::atomic_load(&m_head)); }

  const auto front() const { return ref(std::atomic_load(&m_head)); }
  ...
  void insert_after(iterator position, const T& t) {
    ...
    do {
      ...
      if (std::atomic_load(&m_head) == nullptr) {
        exchanged = std::atomic_compare_exchange_weak(&m_head, &p->next, p);
      } else {
        exchanged =
          std::atomic_compare_exchange_weak(&position.m_data->next, &p->next,
            p);
      }
    } while (!exchanged);
  }
      ...
      do {
        if (p) {
          ...
          if (p && p == head) {
            ...
            exchanged = std::atomic_compare_exchange_weak(&prev->next, &p,
              std::atomic_load(&p->next));
          }
        }
      } while (p && !exchanged);
  ...
  template <class... Args>
  void emplace_front(Args&& ... args) { push_front(std::move(T(args...))); }
  ...
  bool empty() const noexcept { return std::atomic_load(&m_head) == nullptr; }
  ...
  iterator begin() noexcept { return std::atomic_load(&m_head); }
  ...
  const_iterator begin() const noexcept { return std::atomic_load(&m_head); }
